...
function scheduleSaveSettings(){clearTimeout(saveSettingsTimer);saveSettingsTimer=setTimeout(()=>saveSettings(),250)}
...
async function setScoringMode(mode){DPP.settings.scoringMode=mode;saveLocalSettings();updateModeUI();await saveRemoteSettings();if(DPP.participants.length)await createEmptyScores(DPP.participants);await refreshAll();}
