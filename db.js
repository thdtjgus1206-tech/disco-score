...
function subscribeRealtime(){sb.channel('dpp-v4').on('postgres_changes',{event:'*',schema:'public',table:'dpp_scores'},refreshScoresOnly).on('postgres_changes',{event:'*',schema:'public',table:'dpp_participants'},refreshAll).on('postgres_changes',{event:'*',schema:'public',table:'dpp_logs'},async()=>{DPP.logs=await fetchLogs();renderAllAdminViews()}).subscribe()}
